...
- 区分免费、Go、Plus、5x Pro 和 20x Pro 套餐，并在底部居中显示周额度耗尽预测。
...
- **预测状态：** 用绿色显示“用量充裕”，用红色显示预计耗尽倒计时，额度用完时明确显示“额度耗尽”。
...
在 Windows 11 24H2 x64、v0.3.0 Release 版上连续采样 121 秒：

| 状态 | CodexStatus 工作集 | CPU | 子进程 |
|---|---:|---:|---:|
| 额度与服务状态刷新后空闲 | 平均 6.26 MB / 峰值 6.32 MB | 平均 ≤0.1% | 0 |
| 刷新中 | 托盘主进程 <15 MB | 短暂活动 | 1 个临时 `codex app-server` 进程树 |

采样结束时句柄数少于开始值，GDI 与 USER 对象数均未增长。app-server 是 Codex 本身，刷新时会有更大的瞬时占用；完成两个账户查询后立刻退出，不属于常驻托盘进程。
