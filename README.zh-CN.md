...
采样结束时仅有 2 个线程，句柄数少于开始值，GDI 与 USER 对象数均未增长，也没有子进程。Direct2D 渲染目标、画刷和 DirectWrite 文本格式只在卡片可见时创建；关闭卡片后立即释放这些对象，并在稍后主动回收工作集。app-server 是 Codex 本身，刷新时会有更大的瞬时占用；完成两个账户查询后立刻退出，不属于常驻托盘进程。如果 Direct2D 初始化失败，上一版 GDI 绘制路径仍会自动兜底。
